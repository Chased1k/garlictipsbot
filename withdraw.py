...
class withdraw():
    ...
    def process_withdrawal(self,address,amount,username,coin):
        if amount.startswith("."):
            amount = "0"+amount
            #pdb.set_trace()
        txid = subprocess.check_output(shlex.split('usr/local/bin/quantisd sendtoaddress %s %s' % (address, amount)))
        qcheck = subprocess.check_output(shlex.split('%s/%s/bin/%sd sendtoaddress %s %s' % (self.utils.config['other']['full_dir'],coin,coin,address,amount)))
